...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # if preconditions are satisfied in previous state level
            if action_node.prenodes.issubset(self.s_levels[level]):
                # allow the action node to be added to the list of action nodes
                action_nodes.append(action_node)
                # This action node then becomes the child of the state nodes
                # at the preceding level.
                # The state nodes become the parent of the acion node
                for state_node in self.s_levels[level]:
                    state_node.children.add(action_node)
                    action_node.parents.add(state_node)
        # Create the action level from the accessible action nodes
    ...
    def add_literal_level(self, level):
        ...
        state_nodes = set()
        # Get the effect nodes for each preceding action
        for action_node in self.a_levels[level-1]:
            for state_node in action_node.effnodes:
                # Add the effect node to the state node set
                state_nodes.add(state_node)
                # Make the state node the parent of the effect node
                state_node.parents.add(action_node)
                # Make the action node the child of the state node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Ensure node_a1 positive effects not canceled by node_a2 negative effects
        for e in node_a1.action.effect_add:
            if e in node_a2.action.effect_rem:
                return True
        # Ensure node_a2 positive effects not canceled by node_a1 regative effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Ensure the positive effects do not interfere with the corresponding node's negative preconditions
        for e in node_a1.action.effect_add:
        ...
        # Ensure the negative effects do not interfere with the corresponding node's positive preconditions
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check for mutual exclusivity between the preconditions of nodes a1 and a2
        for precon_1 in node_a1.parents:
            for precon_2 in node_a2.parents:
                if precon_1.is_mutex(precon_2):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # Ensure preconditions between nodes are not mutually exclusive
        ...
        for goal in self.problem.goal:
            ...
            for l in range(len(self.s_levels)):
                for s in self.s_levels[l]:
                    if goal == s.symbol and s not in visitedStates:
                        found_goal = True
                        level_sum += l
                        visitedStates.add(s)
                        break
